...
func newReservationsCmd(flags *rootFlags) *cobra.Command {
	...
	cmd.AddCommand(newReservationsHistoryCmd(flags))
...
func newReservationsHistoryCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		Use:   "history",
		Short: "Show your full reservation log (past, upcoming, and external)",
		Long: "Returns every reservation on file for your account — past and\n" +
			"upcoming, Spot-booked and external reservations synced from linked\n" +
			"booking platforms. Sorted most-recent first. For upcoming-only,\n" +
			"use 'spot reservations list'.",
		RunE: func(cmd *cobra.Command, _ []string) error {
			client, err := newClient()
			if err != nil {
				return err
			...
			reservations, err := client.Reservations.History(cmd.Context())
			if err != nil {
				return err
			}
			...
			if len(reservations) == 0 {
				_, _ = fmt.Fprintln(cmd.OutOrStdout(), "No reservations on file.")
				return nil
			}

			tw := render.Table(cmd.OutOrStdout())
			_, _ = fmt.Fprintln(tw, "ID\tRESTAURANT\tDATE\tTIME\tPARTY\tSEATING\tPLATFORM")
			for _, r := range reservations {
				restaurantName := "—"
				if r.Table.Restaurant != nil && r.Table.Restaurant.Name != "" {
					restaurantName = r.Table.Restaurant.Name
				...
				_, _ = fmt.Fprintf(tw, "%s\t%s\t%s\t%s\t%d\t%s\t%s\n",
					...
					r.Table.Party,
					seating,
					platformDisplayName(r.Table.Platform),
				)
			}
			return tw.Flush()
		},
